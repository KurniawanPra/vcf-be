...
class VcfBagian3Controller extends Controller
{
    ...
    public function store(Request $request, int $vcfId)
    {
        ...
        if ($vcf->status !== 'bagian2_selesai') {
            return response()->json([
                'message' => 'Bagian 3 hanya dapat diisi setelah Bagian 2 (Timbangan Masuk) selesai.',
    ...
    public function reject(Request $request, int $vcfId)
    {
        ...
        if ($vcf->status !== 'bagian2_selesai') {
            return response()->json([
                'message' => 'Hanya VCF yang sudah melewati tahap 2 yang dapat di-reject di tahap ini.',
            ], 422);
        }
